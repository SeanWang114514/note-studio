// 极窄窗口：模拟用户截图里页面右侧被截断的情况，检查 fit-width 与 span 对齐
import { spawn } from 'node:child_process'
import { writeFileSync } from 'node:fs'

const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe'
const PORT = 9502
const profile = process.env.TEMP + '/chrome-vnarrow-' + PORT
const chrome = spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-first-run', '--window-size=1000,547', '--user-data-dir=' + profile, '--remote-debugging-port=' + PORT, 'about:blank'], { stdio: 'ignore' })
...
async function main() {
  ...
  console.log('VNARROW:', dump.result?.value)

  const shot = await send('Page.captureScreenshot', { format: 'png' })
  const data = shot?.data || shot?.result?.data
  if (data) writeFileSync('D:/VibeCoding/note apps/pdf-vnarrow.png', Buffer.from(data, 'base64'))
