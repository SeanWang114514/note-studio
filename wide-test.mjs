// 模拟用户截图1环境：宽窗口1548，检查 fit-width 是否生效、页面是否被截断
import { spawn } from 'node:child_process'
import { writeFileSync } from 'node:fs'

const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe'
const PORT = 9496
const profile = process.env.TEMP + '/chrome-wide-' + PORT
const chrome = spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-first-run', '--window-size=1548,900', '--user-data-dir=' + profile, '--remote-debugging-port=' + PORT, 'about:blank'], { stdio: 'ignore' })
...
async function main() {
  ...
  const dump = await send('Runtime.evaluate', { expression: `(() => {
    ...
    return JSON.stringify({
      ...
      ribbonPct: [...document.querySelectorAll('.ribbon-pct, .status-bar span')].map(e => e.textContent).join(' ')
    })
  })()`, returnByValue: true })
  console.log('WIDE:', dump.result?.value)

  const shot = await send('Page.captureScreenshot', { format: 'png' })
  const data = shot?.data || shot?.result?.data
  if (data) writeFileSync('D:/VibeCoding/note apps/pdf-wide.png', Buffer.from(data, 'base64'))
