...
- **全格式「新建一页」+「删除新建页」**（删除 = 撤销新建，两次点击确认，见下方「新建一页 / 删除新建页」）

## 新建一页 / 删除新建页

工具栏上的「新建一页」按当前格式的语义追加一页；紧挨着的「删除新建页」把它撤销掉 ——
**只删自己这一会话里新建的页，文件原有的页一张都不会少**。「一页」在各格式里的含义：

| 格式 | 「新建一页」做的 | 「删除新建页」做的 |
| --- | --- | --- |
| PDF | 末尾追加一张真实空白页（pdf-lib 改文档结构） | 删掉末尾那一页 |
| Word（docx） | 文末插入 Word 分页符 | 移掉文末那个分页标记 |
| Markdown | 追加 `\pagebreak` 分页块 + 一个空段落 | 按块 id 移掉这两块 |
| EPUB | 书末追加一个新章节（登记进 OPF） | 把新章节移出书末，OPF 里也不再登记 |
| Excel | 新建一张工作表 | 删掉那张工作表 |
| 纯文本 | 在光标处插入换页符 `^L` | 移除刚插入的那个换页符 |
| PPT / CAJ / 其它（白板） | 白板后面追加一张空白页 | 页数退回「本会话第一次加页前」的值 |

- **二级确定，不弹窗**（`src/components/DeletePageButton.jsx`）：第一次点击只是「上膛」——
  按钮变成红底「确认删除」，**3 秒内再点一次**才真的执行；期间点到别处（失焦）、超时、
  按钮被置灰都会自动收回。挨着「新建一页」的按钮误点一下，不会删掉任何东西。
- **有东西可撤销才亮**：没点过「新建一页」时按钮是灰的；白板格式还额外受「文件原有页数」
  下界保护，删不回文件里本来没有的页。
- **后进先出**（`src/lib/useNewPageUndo.js`）：连点两次「新建一页」就要连点两次删除，
  每次删掉最近加的那一页；换文件后记账清零。
- **批注不跟着页走**：删除前按「旧高/新高」把既有批注纵向重标定一次，画在其它页上的墨迹
  仍停在自己原来那一页；原本画在被删掉那页上的墨迹不随之删除，只是跟着页面移出可视区。
- 窄屏（手机）下按钮文字照旧收起（与「新建一页」同约定），但上膛后「确认删除」四个字会露出来。
