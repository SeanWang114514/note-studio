// 测试 actualSize(100%) 与编辑态下 span/画布对齐；并检查 fit-width 首次打开时序
import { spawn } from 'node:child_process'

const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe'
const PORT = 9494
const profile = process.env.TEMP + '/chrome-100pct-' + PORT
...
async function main() {
  ...
  // 点实际大小（100%）
  await send('Runtime.evaluate', { expression: `[...document.querySelectorAll('.ribbon-btn')].find(b => (b.title || '').includes('实际大小'))?.click()`, returnByValue: true })
  await sleep(1500)
  ...
  const align = await send('Runtime.evaluate', { expression: `(() => {
    ...
    const bands = []
    let cur = null
    for (const y of rowDark) { if (!cur || y - cur.end > 4) { cur = { start: y, end: y }; bands.push(cur) } else cur.end = y }
    ...
    return JSON.stringify({
      dpr,
      page: { w: Math.round(pr.width), left: Math.round(pr.left - document.body.getBoundingClientRect().left), right: Math.round(pr.right - document.body.getBoundingClientRect().left) },
      scroll: { clientW: scroll.clientWidth, left: Math.round(scroll.getBoundingClientRect().left - document.body.getBoundingClientRect().left), right: Math.round(scroll.getBoundingClientRect().right - document.body.getBoundingClientRect().left) },
      pageInside: pr.right <= scroll.getBoundingClientRect().right + 1,
      canvasBands: canvasBands.slice(0, 8),
  ...
  console.log('100PCT:', align.result?.value)
