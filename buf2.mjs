// 决定性：读取 canvas buffer 上标题文字的实际 y 位置 vs span 期望位置
import { spawn } from 'node:child_process'

const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe'
const PORT = 9515
const profile = process.env.TEMP + '/chrome-buf2-' + PORT
...
async function main() {
  ...
  const dump = await send('Runtime.evaluate', { expression: `(() => {
    ...
    // 全宽扫描找文字带（buffer 物理像素）
    const rowDark = []
    for (let y = 0; y < H; y += 2) {
      const img = ctx.getImageData(0, y, W, 1).data
      let dark = 0
      for (let x = 0; x < W; x += 4) { const i = x * 4; if ((img[i] + img[i+1] + img[i+2]) / 3 < 150) dark++ }
      if (dark > 5) rowDark.push(y)
    }
    const bands = []
    let cur = null
    for (const y of rowDark) { if (!cur || y - cur.end > 8) { cur = { start: y, end: y }; bands.push(cur) } else cur.end = y }
    const canvasBufLines = bands.map(b => Math.round((b.start + b.end) / 2)).slice(0, 8)
    // span 期望的 buffer 位置
    const grab = (t) => { const s = [...tl.querySelectorAll('span[data-page="1"]')].find(x => (x.textContent || '').includes(t)); if (!s) return null; const r = s.getBoundingClientRect(); return Math.round((r.top - pr.top + r.height / 2) * dpr) }
    const spanBufExpected = { title: grab('研究性'), line1: grab('所有同学'), sheng: grab('生登录'), star: grab('★') }
    return JSON.stringify({
      ...
      ratio: spanBufExpected && spanBufExpected.title ? (canvasBufLines[0] / spanBufExpected.title).toFixed(3) : null,
    })
  })()`, returnByValue: true })
  console.log('BUF2:', dump.result?.value)
