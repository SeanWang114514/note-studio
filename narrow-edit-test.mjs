// 窄窗口 + 编辑态：像素级对比 span 与画布，确认错位
import { spawn } from 'node:child_process'
import { writeFileSync } from 'node:fs'

const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe'
const PORT = 9493
const profile = process.env.TEMP + '/chrome-narrow-edit-' + PORT
...
async function main() {
  ...
  const cmp = await send('Runtime.evaluate', { expression: `(() => {
    ...
    for (let y = 0; y < H; y += 2) {
      ...
      for (let x = 0; x < W; x += 3) {
        const i = x * 4
        if ((img[i] + img[i+1] + img[i+2]) / 3 < 150) dark++
      }
      if (dark > 3) rowDark.push(y / dpr)
    }
    const bands = []
    let cur = null
    for (const y of rowDark) {
      if (!cur || y - cur.end > 4) { cur = { start: y, end: y }; bands.push(cur) } else cur.end = y
    }
  ...
  console.log('NARROW-EDIT:', cmp.result?.value)

  const shot = await send('Page.captureScreenshot', { format: 'png' })
  const data = shot?.data || shot?.result?.data
  if (data) writeFileSync('D:/VibeCoding/note apps/pdf-narrow-edit.png', Buffer.from(data, 'base64'))
