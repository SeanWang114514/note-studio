// 检查打开 PDF 的时序：mount 时 scale=1 布局 → fitWidth 缩小，中间可能被截图
import { spawn } from 'node:child_process'
import { writeFileSync } from 'node:fs'

const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe'
const PORT = 9498
const profile = process.env.TEMP + '/chrome-mount-timing-' + PORT
...
async function main() {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const s = await send('Runtime.evaluate', { expression: `(() => {
      ...
      if (!page) return JSON.stringify({ t: ${i}, noPage: true })
      const pr = page.getBoundingClientRect()
      const scroll = document.querySelector('.pdf-scroll')
      const sr = scroll.getBoundingClientRect()
      const tl = document.querySelector('.pdf-text-layer')
      const status = [...document.querySelectorAll('.status-bar span')].map(e => e.textContent).join('|')
      return JSON.stringify({
        t: ${i},
        status,
        scaleFactor: tl?.style.getPropertyValue('--scale-factor') || 'none',
