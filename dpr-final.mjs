// 决定性验证：DPR=1.25 下保存画布纯图 + 全部 span 位置，精确对比
import { spawn } from 'node:child_process'
import { writeFileSync } from 'node:fs'

const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe'
const PORT = 9507
const profile = process.env.TEMP + '/chrome-dpr-final-' + PORT
...
async function main() {
  ...
  if (b64) writeFileSync('D:/VibeCoding/note apps/pdf-canvas-dpr125.png', Buffer.from(b64.split(',')[1], 'base64'))

  // 全部 span 位置 vs 画布全宽文字带
  const cmp = await send('Runtime.evaluate', { expression: `(() => {
    ...
    const spans = [...tl.querySelectorAll('span[data-page="1"]')].map(s => {
      ...
      return { idx: s.dataset.idx, text: (s.textContent || '').slice(0, 10), y: Math.round((r.top - pr.top + r.height / 2) * 10) / 10, editing: s.classList.contains('editing') }
    }).filter(s => s.text && s.text.trim())
    return JSON.stringify({ dpr, scaleFactor: tl.style.getPropertyValue('--scale-factor'), canvasBands: canvasBands.slice(0, 12), spans: spans.slice(0, 25), status: [...document.querySelectorAll('.status-bar span')].map(e => e.textContent).join('|') })
  })()`, returnByValue: true })
  console.log('FINAL:', cmp.result?.value)
