// diag-num.mjs — 点击 5.填写 段落，验证 55 问题
...
const userData = path.join(os.tmpdir(), 'diag-num-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9253', '--user-data-dir=' + userData, '--no-first-run', '--disable-gpu', '--disable-extensions', '--window-size=1600,1000', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9253/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
if (!info.found) { console.log('5.填写 not found'); chrome.kill(); process.exit(0) }
