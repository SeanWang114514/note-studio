...
const userData = path.join(os.tmpdir(), 'commit-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9307', '--user-data-dir=' + userData, '--no-first-run', '--no-sandbox', '--disable-gpu', '--disable-extensions', '--window-size=1600,1200', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9307/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
// 编辑前截图
  const shotBefore = await cdp.send('Page.captureScreenshot', { format: 'png' })
  fs.writeFileSync(path.join(ROOT, 'tmp', 'before-edit.png'), Buffer.from(shotBefore.data, 'base64'))
  console.log('screenshot: tmp/before-edit.png')
  // 点击'所有同学'
...
await sleep(800)
// 修改文字
await cdp.eval('(function(){ var box = document.querySelector(".pdf-inline-editor"); if (!box) return "no box"; box.focus(); var sel = window.getSelection(); sel.selectAllChildren(box); document.execCommand("insertText", false, "修改后文字"); return "done" })()')
await sleep(300)
// 提交（点击空白处）
await cdp.send('Input.dispatchMouseEvent', { type: 'mousePressed', x: 100, y: 1000, button: 'left', clickCount: 1 })
await cdp.send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 100, y: 1000, button: 'left', clickCount: 1 })
await sleep(1000)
// 检查 edit-canvas 内容和尺寸
const edState = await cdp.eval('(function(){ var page = [...document.querySelectorAll(".pdf-page")].find(x => x.dataset.page === "1"); if (!page) return null; var ec = page.querySelector(".pdf-edit-canvas"); var layer = page.querySelector(".pdf-text-layer"); var ecr = ec.getBoundingClientRect(); var lr = layer.getBoundingClientRect(); return { editCanvasCss: { w: ecr.width, h: ecr.height }, textLayerCss: { w: lr.width, h: lr.height }, canvasAttr: { w: ec.width, h: ec.height }, editedSpans: page.querySelectorAll(".pdf-text-layer span.edited").length } })()')
console.log('EDIT STATE:', JSON.stringify(edState, null, 1))
const shot = await cdp.send('Page.captureScreenshot', { format: 'png' })
fs.writeFileSync(path.join(ROOT, 'tmp', 'commit-fix.png'), Buffer.from(shot.data, 'base64'))
console.log('screenshot: tmp/commit-fix.png')
