...
const userData = path.join(os.tmpdir(), 'zoom-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9302', '--user-data-dir=' + userData, '--no-first-run', '--no-sandbox', '--disable-gpu', '--disable-extensions', '--window-size=1600,1200', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9302/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
// 找到当前缩放控制并设置大缩放 (300%)
// 先看缩放按钮
const zoomInfo = await cdp.eval('(function(){ var b = [...document.querySelectorAll("button")].find(x => (x.textContent||"").trim().endsWith("%")); return b ? { found: true, text: b.textContent.trim() } : { found: false } })()')
console.log('ZOOM BTN:', JSON.stringify(zoomInfo))
// 点击缩放按钮看菜单
if (zoomInfo.found) {
  await cdp.eval('(function(){ var b = [...document.querySelectorAll("button")].find(x => (x.textContent||"").trim().endsWith("%")); b.click(); return "ok" })()')
  await sleep(500)
  const menu = await cdp.eval('(function(){ var opts = [...document.querySelectorAll("button, [role=menuitem], li, option")].map(x => x.textContent.trim()).filter(t => t.includes("%")); return opts })()')
  console.log('ZOOM MENU:', JSON.stringify(menu))
}
