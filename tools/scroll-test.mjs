...
const userData = path.join(os.tmpdir(), 'scrl-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9303', '--user-data-dir=' + userData, '--no-first-run', '--no-sandbox', '--disable-gpu', '--disable-extensions', '--window-size=1600,1200', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9303/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
// 模拟用户滚动：向下滚动一半页面
await cdp.send('Input.dispatchMouseEvent', { type: 'mouseWheel', x: 800, y: 600, deltaY: 800, deltaX: 0 })
await sleep(500)
await cdp.send('Input.dispatchMouseEvent', { type: 'mouseWheel', x: 800, y: 600, deltaY: 800, deltaX: 0 })
await sleep(800)
// 检查滚动位置
const scrollInfo = await cdp.eval('(function(){ var sc = document.querySelector(".pdf-scroll-container, .pdf-viewer-body, [class*=scroll]"); if (!sc) return { noScroll: true }; return { scrollTop: sc.scrollTop, scrollHeight: sc.scrollHeight, clientHeight: sc.clientHeight } })()')
console.log('SCROLL:', JSON.stringify(scrollInfo))
// 找一个可见的文字 span 点击
const info = await cdp.eval('(function(){ var spans = document.querySelectorAll(".pdf-text-layer span[data-page]"); for (var i=0;i<spans.length;i++){ var s = spans[i]; var t = s.textContent || ""; if (t.indexOf("所有同学") >= 0 || t.indexOf("个人课题") >= 0) { var r = s.getBoundingClientRect(); if (r.top > 50 && r.top < 1000 && r.left > 0) { return { found: true, text: t.slice(0,20), click: { x: r.left + r.width/2, y: r.top + r.height/2 }, spanRect: { left: r.left, top: r.top, width: r.width, height: r.height } } } } } return { found: false } })()')
...
await sleep(1000)
// 检查编辑框位置
const ed = await cdp.eval('(function(){ var pos = document.querySelector(".pdf-inline-editor-pos"); if (!pos) return { editorOpen: false }; var box = document.querySelector(".pdf-inline-editor"); var pr = pos.getBoundingClientRect(); var pageEl = pos.closest(".pdf-page"); var layer = pos.closest(".pdf-text-layer") || document.querySelector(".pdf-text-layer"); var lr = layer ? layer.getBoundingClientRect() : null; return { editorOpen: true, posRect: { left: pr.left, top: pr.top, width: pr.width, height: pr.height }, layerRect: lr ? { left: lr.left, top: lr.top, width: lr.width, height: lr.height } : null, pageRect: pageEl ? pageEl.getBoundingClientRect().top : null, viewportH: window.innerHeight } })()')
console.log('EDITOR POS:', JSON.stringify(ed, null, 1))
const shot = await cdp.send('Page.captureScreenshot', { format: 'png' })
fs.writeFileSync(path.join(ROOT, 'tmp', 'scroll-edit.png'), Buffer.from(shot.data, 'base64'))
console.log('screenshot: tmp/scroll-edit.png')
