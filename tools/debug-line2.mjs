...
const EV = fs.readFileSync(path.join(__dirname, 'debug-ev.txt'), 'utf8').trim()
const userData = path.join(os.tmpdir(), 'dbgl2-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9270', '--user-data-dir=' + userData, '--no-first-run', '--disable-gpu', '--disable-extensions', '--window-size=1600,1000', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9270/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
await sleep(15000)
const info = await cdp.eval('(function() { var spans = document.querySelectorAll("[data-page]"); for (var i = 0; i < spans.length; i++) { var s = spans[i]; var t = s.textContent || ""; if (t.length > 5 && /[\u4e00-\u9fff]/.test(t)) { s.scrollIntoView(); var r = s.getBoundingClientRect(); return { found: true, click: { x: r.left + r.width/2, y: r.top + r.height/2 }, text: t.slice(0,20) } } } return { found: false } })()')
