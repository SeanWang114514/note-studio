...
const userData = path.join(os.tmpdir(), 'diag-manual4-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9257', '--user-data-dir=' + userData, '--no-first-run', '--disable-gpu', '--disable-extensions', '--window-size=1600,1000', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9257/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
await sleep(9000)
await cdp.eval('(() => { const layers = [...document.querySelectorAll(".pdf-text-layer")]; for (const layer of layers) { const spans = [...layer.querySelectorAll("span[data-page]")]; const s = spans.find(x => (x.textContent||"") === "相应内容，"); if (s) { s.scrollIntoView({ block: "center" }); return { found: true, page: s.dataset.page, text: s.textContent } } } return { found: false } })()')
await sleep(800)
const info = await cdp.eval('(() => { const layers = [...document.querySelectorAll(".pdf-text-layer")]; for (const layer of layers) { const spans = [...layer.querySelectorAll("span[data-page]")]; const s = spans.find(x => (x.textContent||"") === "相应内容，"); if (s) { const r = s.getBoundingClientRect(); return { found: true, page: s.dataset.page, text: s.textContent, click: { x: r.left + r.width / 2, y: r.top + r.height / 2 }, spanRect: { left: r.left, top: r.top, width: r.width, height: r.height } } } } return { found: false } })()')
console.log('TARGET:', JSON.stringify(info, null, 1))
if (!info.found) { console.log('not found'); chrome.kill(); process.exit(0) }
