// real-rich2.mjs — use 5173 prod server
...
const userData = path.join(os.tmpdir(), 'rich2-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9267', '--user-data-dir=' + userData, '--no-first-run', '--disable-gpu', '--disable-extensions', '--window-size=1600,1000', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9267/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
await sleep(8000)
const dbg = await cdp.eval('(function() { return { pageCount: document.querySelectorAll(".pdf-page").length, spanCount: document.querySelectorAll("span[data-page]").length, hasError: !!document.querySelector(".pdf-error, .pdf-loading"), consoleErrors: window.__consoleErrors || [] } })()')
console.log("DBG:", JSON.stringify(dbg))
if (!dbg || dbg.pageCount === 0) {
  const errs = await cdp.eval('(function() { return (window.__pdfErrors || []).slice(-5) })()')
  console.log("ERRORS:", JSON.stringify(errs));
  const html = await cdp.eval('(function() { return document.body.innerHTML.slice(0, 500) })()')
  console.log("BODY:", html);
  cdp.close(); chrome.kill(); await sleep(800); try { fs.rmSync(userData, { recursive: true, force: true }) } catch {}; process.exit(0)
}
...
if (!info.found) { cdp.close(); chrome.kill(); await sleep(800); try { fs.rmSync(userData, { recursive: true, force: true }) } catch {}; process.exit(0) }
...
fs.writeFileSync(path.join(ROOT, 'tmp', 'real-rich2.png'), Buffer.from(shot.data, 'base64'))
console.log("screenshot: tmp/real-rich2.png")
