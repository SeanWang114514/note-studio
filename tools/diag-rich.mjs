...
const EV = fs.readFileSync(path.join(__dirname, 'rich-ev.txt'), 'utf8').trim()
class CDP {
...
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9262', '--user-data-dir=' + userData, '--no-first-run', '--disable-gpu', '--disable-extensions', '--window-size=1600,1000', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9262/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
await sleep(9000)
// 滚到第3页 + 点击相应内容
await cdp.eval('(() => { const pages = [...document.querySelectorAll(".pdf-page")]; const p3 = pages.find(x => x.dataset.page === "3"); p3 && p3.scrollIntoView({block: "center"}); return p3 ? "ok" : "not found" })()')
await sleep(800)
...
await sleep(1000)
const dump = await cdp.eval(EV)
console.log('RICH DUMP:', JSON.stringify(dump, null, 1))
const shot = await cdp.send('Page.captureScreenshot', { format: 'png' })
fs.writeFileSync(path.join(ROOT, 'tmp', 'diag-rich.png'), Buffer.from(shot.data, 'base64'))
console.log('screenshot: tmp/diag-rich.png')
