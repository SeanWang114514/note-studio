...
const userData = path.join(os.tmpdir(), 'repro2-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9301', '--user-data-dir=' + userData, '--no-first-run', '--no-sandbox', '--disable-gpu', '--disable-extensions', '--window-size=1600,1200', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9301/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
await sleep(1500)
// 点击'★个人课题'所在行
const info = await cdp.eval('(function(){ var spans = document.querySelectorAll(".pdf-text-layer span[data-page]"); for (var i=0;i<spans.length;i++){ var s = spans[i]; var t = s.textContent || ""; if (t.indexOf("研究性") >= 0 && t.length < 20) { var r = s.getBoundingClientRect(); return { found: true, text: t.slice(0,30), click: { x: r.left + r.width/2, y: r.top + r.height/2 } } } } return { found: false } })()')
...
await sleep(1200)
// 检查编辑框
const ed = await cdp.eval('(function(){ var pos = document.querySelector(".pdf-inline-editor-pos"); var box = document.querySelector(".pdf-inline-editor"); if (!pos) return { editorOpen: false }; var pr = pos.getBoundingClientRect(); var br = box ? box.getBoundingClientRect() : null; var lines = box ? box.querySelectorAll("[data-line]") : []; var out = { editorOpen: true, posRect: { left: pr.left, top: pr.top, width: pr.width, height: pr.height }, boxRect: br ? { left: br.left, top: br.top, width: br.width, height: br.height } : null, lineCount: lines.length, text: box ? box.textContent.slice(0, 60) : null }; if (lines.length) { out.lineInfo = []; for (var i = 0; i < lines.length; i++) { var lr = lines[i].getBoundingClientRect(); var cs = getComputedStyle(lines[i]); var spans = lines[i].querySelectorAll("span"); var spanInfo = []; for (var j = 0; j < spans.length; j++) { var sr = spans[j].getBoundingClientRect(); spanInfo.push({ text: spans[j].textContent.slice(0,10), left: Math.round(sr.left), width: Math.round(sr.width) }) } out.lineInfo.push({ i: i, text: lines[i].textContent.slice(0,30), left: Math.round(lr.left), width: Math.round(lr.width), transform: cs.transform, spans: spanInfo }) } } return out })()')
console.log('EDITOR:', JSON.stringify(ed, null, 1))
const shot = await cdp.send('Page.captureScreenshot', { format: 'png' })
fs.writeFileSync(path.join(ROOT, 'tmp', 'repro2-star.png'), Buffer.from(shot.data, 'base64'))
console.log('screenshot: tmp/repro2-star.png')
