...
const userData = path.join(os.tmpdir(), 'edcan-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9306', '--user-data-dir=' + userData, '--no-first-run', '--no-sandbox', '--disable-gpu', '--disable-extensions', '--window-size=1600,1200', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9306/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
// 检查第1页所有 canvas 尺寸
const EV = fs.readFileSync(path.join(__dirname, 'canvas-ev.txt'), 'utf8').trim()
  const sizes = await cdp.eval(EV)
console.log('CANVAS SIZES:', JSON.stringify(sizes, null, 1))
