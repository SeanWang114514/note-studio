...
const EV = fs.readFileSync(path.join(__dirname, 'diag-ev.txt'), 'utf8').trim()
const userData = path.join(os.tmpdir(), 'diag-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9272', '--user-data-dir=' + userData, '--no-first-run', '--disable-gpu', '--disable-extensions', '--window-size=1600,1000', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9272/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
fs.writeFileSync(path.join(ROOT, 'tmp', 'diag-all.png'), Buffer.from(shot.data, 'base64'))
console.log("screenshot: tmp/diag-all.png")
