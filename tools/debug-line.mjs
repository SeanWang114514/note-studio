// debug-line.mjs — 验证 data-line 和 lineIdx
...
const userData = path.join(os.tmpdir(), 'debug-' + Date.now())
fs.mkdirSync(userData, { recursive: true })
const chrome = spawn(CHROME, ['--headless=new', '--remote-debugging-port=9269', '--user-data-dir=' + userData, '--no-first-run', '--disable-gpu', '--disable-extensions', '--window-size=1600,1000', 'about:blank'], { stdio: 'ignore' })
let targets = []
for (let i = 0; i < 50; i++) { try { const res = await fetch('http://127.0.0.1:9269/json/list'); targets = await res.json(); if (targets.length) break } catch {}; await sleep(200) }
...
await sleep(7000)
const dbg = await cdp.eval('(function() { var layers = document.querySelectorAll(".pdf-text-layer"); var out = { layerCount: layers.length }; if (layers.length > 0) { var spans = layers[0].querySelectorAll("[data-page]"); var sample = []; for (var i = 0; i < Math.min(spans.length, 10); i++) { var s = spans[i]; sample.push({ text: s.textContent.slice(0,10), lineIdx: s.dataset.lineIdx, page: s.dataset.page }) } out.sample = sample } return out })()')
console.log("DBG:", JSON.stringify(dbg))
const shot = await cdp.send('Page.captureScreenshot', { format: 'png' })
fs.writeFileSync(path.join(ROOT, 'tmp', 'debug-line.png'), Buffer.from(shot.data, 'base64'))
console.log("screenshot: tmp/debug-line.png")
